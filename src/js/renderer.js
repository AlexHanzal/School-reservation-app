            ...
            for (let dayIndex = 0; dayIndex < daysToProcess; dayIndex++) {
                ...
                Object.entries(dayData).forEach(([hourIndex, hourObj]) => {
                    ...
                    if (hourObj && typeof hourObj === 'object' && hourObj.content) {
                        ...
                        // Show abbreviation if NOT permanent and hourObj.abbreviation exists
                        if (!hourObj.isPermanent && hourObj.abbreviation) {
                            ...
                            span.addEventListener('keypress', e => e.stopPropagation());
                            span.addEventListener('input', e => e.stopPropagation());
                            span.addEventListener('paste', e => e.preventDefault());
                            span.addEventListener('cut', e => e.preventDefault());
                            span.addEventListener('copy', e => e.preventDefault());
...
function shouldApplyPermanentHour(hourObj, weekDate) {
    // Only check isPermanent
    return !!hourObj.isPermanent;
}
    ...
    rows.forEach((row, dayIndex) => {
        ...
        cells.forEach((cell, hourIndex) => {
            ...
            if (content !== '') {
                ...
                if (abbreviation && !isPermanent) {
                    hourObj.abbreviation = abbreviation;
                }
...
function setupCellEditing() {
    ...
    cells.forEach(cell => {
        ...
        // Helper to get cell content without abbreviation
        ...
        // Helper to set cell content and abbreviation
        ...
        // --- Begin: Prevent editing other's cells ---
        // Only allow editing if:
        // - Admin mode is enabled, OR
        // - The cell is empty, OR
        // - The cell's abbreviation matches the current user's abbreviation
        let canEdit = false;
        const abbrevSpan = newCell.querySelector('.user-abbreviation');
        if (isAdminMode) {
            canEdit = true;
            newCell.classList.remove('locked-cell');
            ...
            abbrevSpan.textContent === currentUser.abbreviation
        ) {
            canEdit = true;
            newCell.classList.remove('locked-cell');
        } else {
            canEdit = false;
            newCell.classList.add('locked-cell');
        }
        newCell.setAttribute('contenteditable', canEdit ? 'true' : 'false');
        // --- End: Prevent editing other's cells ---

        // Make any abbreviation span inside the cell non-editable and block events
        const abbrevSpanElem = newCell.querySelector('.user-abbreviation');
        if (abbrevSpanElem) {
            abbrevSpanElem.setAttribute('contenteditable', 'false');
            abbrevSpanElem.addEventListener('keydown', e => e.stopPropagation());
            abbrevSpanElem.addEventListener('keypress', e => e.stopPropagation());
            abbrevSpanElem.addEventListener('input', e => e.stopPropagation());
            abbrevSpanElem.addEventListener('paste', e => e.preventDefault());
            abbrevSpanElem.addEventListener('cut', e => e.preventDefault());
            abbrevSpanElem.addEventListener('copy', e => e.preventDefault());
            abbrevSpanElem.addEventListener('mousedown', e => e.stopPropagation());
            abbrevSpanElem.addEventListener('mouseup', e => e.stopPropagation());
