...
async function handleLogin() {
    ...
    try {
        ...
        if (response.ok) {
            ...
            // IMPORTANT: Remove not-logged-in class from body BEFORE loading timetables
            document.body.classList.remove('not-logged-in');
            
            // Remove the login required message if it exists
            const loginMessage = document.querySelector('.login-required-message');
            if (loginMessage) {
                loginMessage.remove();
            }

            // Remove the calendar placeholder message
            const calendarPlaceholder = document.querySelector('.calendar-placeholder');
            if (calendarPlaceholder) {
                calendarPlaceholder.style.display = 'none';
            }
            
            // Make bottom content and calendar visible
            const bottomContent = document.querySelector('.bottom-content');
            const calendar = document.querySelector('.timetable-calendar');
...
function setupVerificationWindow() {
    ...
    function verifyAdminCode() {
        ...
        if (code === '1918') {
            ...
            // FIX: Don't activate admin mode if code is wrong
            isAdminMode = false; 
...
async function loadTimetables() {
    try {
        ...
        if (!currentUser || !currentUser.isLoggedIn) {
            ...
            // Ensure any login required message is removed
...
async function saveTimetable(name, data) {
    try {
        ...
        const dataToBeSaved = JSON.parse(JSON.stringify({
            fileId: data.fileId,
            data: data.data,
        ...
        console.log('Saving timetable data:', dataToBeSaved);
...
async function saveTimeTable() {
    ...
    // Track if any permanent hours were modified
    let permanentHoursModified = false;
    const originalPermanentHours = JSON.stringify(timetables[currentTimetableName].permanentHours || {});
    ...
    rows.forEach((row, dayIndex) => {
        ...
        if (!timetables[currentTimetableName].data[dateString]) {
            ...
            // Initialize with 5 empty arrays (one for each day)
            for (let i = 0; i < 5; i++) {
                timetables[currentTimetableName].data[dateString][i] = [];
            }
        }
        ...
        cells.forEach((cell, hourIndex) => {
            ...
            // Add user abbreviation if cell was edited
            if (cell.classList.contains('edited-cell') && currentUser.isLoggedIn && cellContent !== '') {
                // Only add the abbreviation if it's not already there
                if (!cellContent.includes(`(${currentUser.abbreviation})`)) {
                    cell.textContent = `${cellContent}\n(${currentUser.abbreviation})`;
                }
            }
            
            // Store the content in the data structure
            timetables[currentTimetableName].data[dateString][dayIndex][hourIndex] = cell.textContent.trim();
            
            // If this is a permanent hour, save it to the permanent hours structure
            if (cell.classList.contains('permanent-hour')) {
            ...
            // If cell was previously permanent but isn't anymore, remove it from permanent hours
            else if (timetables[currentTimetableName].permanentHours?.[dayIndex]?.[hourIndex + 1]) {
                delete timetables[currentTimetableName].permanentHours[dayIndex][hourIndex + 1];
                // Clean up empty objects
                if (Object.keys(timetables[currentTimetableName].permanentHours[dayIndex]).length === 0) {
                    delete timetables[currentTimetableName].permanentHours[dayIndex];
                }
            }
            
            // Remove the edited-cell class after processing
            cell.classList.remove('edited-cell');
        });
    });

    // Check if permanent hours were modified
    if (JSON.stringify(timetables[currentTimetableName].permanentHours || {}) !== originalPermanentHours) {
        permanentHoursModified = true;
    }
    ...
    try {
        // If permanent hours were modified, propagate them to future weeks
        if (permanentHoursModified) {
            await propagatePermanentHours(timetables[currentTimetableName]);
        }
        ...
        // If permanent hours were modified, show success message about propagation
        if (permanentHoursModified) {
            showCustomAlert('Success', 'Permanent hours have been saved and propagated to future weeks', 'success');
        } else {
        ...
        showCustomAlert('Error', `Failed to save: ${error.message}`, 'error');
    }
}

/**
 * Propagates permanent hours to future weeks
 * @param {Object} timetable - The timetable object containing permanent hours
 */
async function propagatePermanentHours(timetable) {
    if (!timetable || !timetable.permanentHours) {
        return;
    }
    
    // Define the propagation range (2 months ahead)
    const startDate = new Date(timetable.currentWeek);
    const endDate = new Date(startDate);
    endDate.setMonth(endDate.getMonth() + 2);
    
    console.log(`Propagating permanent hours from ${startDate.toDateString()} to ${endDate.toDateString()}`);
    
    // Iterate through each week in the range
    const currentDate = new Date(startDate);
    const weekMilliseconds = 7 * 24 * 60 * 60 * 1000; // 7 days in milliseconds
    
    // Keep track of modified dates to avoid redundant data
    const modifiedDates = new Set();
    
    while (currentDate <= endDate) {
        // For each day of the week (Monday to Friday)
        for (let dayIndex = 0; dayIndex < 5; dayIndex++) {
            // Skip if no permanent hours for this day
            if (!timetable.permanentHours[dayIndex]) {
                continue;
            }
            ...
            if (!timetable.data[dateString]) {
                timetable.data[dateString] = [];
            }
            
            // Ensure array has entries for all days
            while (timetable.data[dateString].length <= dayIndex) {
                timetable.data[dateString].push([]);
            }
            
            // Apply permanent hours to this day
            for (const hourKey in timetable.permanentHours[dayIndex]) {
                const hourIndex = parseInt(hourKey) - 1;
                const content = timetable.permanentHours[dayIndex][hourKey];
                
                // Ensure the hourIndex position exists in the array
                while (timetable.data[dateString][dayIndex].length <= hourIndex) {
                    timetable.data[dateString][dayIndex].push('');
                }
                
                // Only update if cell is empty or already has the same permanent hour
                const existingContent = timetable.data[dateString][dayIndex][hourIndex];
                if (!existingContent || existingContent === content) {
                    timetable.data[dateString][dayIndex][hourIndex] = content;
                    modifiedDates.add(dateString);
                }
            }
            
            // Clean up empty arrays to keep the data file compact
            cleanupEmptyArrays(timetable.data[dateString], dayIndex);
        }
    ...
    // Clean up empty dates to keep data compact
    for (const dateString of Object.keys(timetable.data)) {
        if (isEmptyDate(timetable.data[dateString])) {
            delete timetable.data[dateString];
        }
    }
    
    console.log(`Modified ${modifiedDates.size} dates with permanent hours`);
}

/**
 * Helper function to clean up empty arrays in the data structure
 * @param {Array} dateData - Array of day data for a specific date
 * @param {number} dayIndex - Index of the day being processed
 */
function cleanupEmptyArrays(dateData, dayIndex) {
    // Skip if dateData is not valid
    if (!dateData || !Array.isArray(dateData)) return;
    
    // Check if the day's array has only empty values
    const dayData = dateData[dayIndex];
    if (!dayData || !Array.isArray(dayData)) return;
    
    // Trim the array by removing trailing empty strings
    let lastNonEmptyIndex = -1;
    for (let i = dayData.length - 1; i >= 0; i--) {
        if (dayData[i] && dayData[i].trim() !== '') {
            lastNonEmptyIndex = i;
            break;
        }
    }
    ...
    if (lastNonEmptyIndex >= 0) {
        // Keep only up to the last non-empty entry
        dateData[dayIndex] = dayData.slice(0, lastNonEmptyIndex + 1);
    } else {
        // If all entries are empty, use an empty array instead
        dateData[dayIndex] = [];
    }
}

/**
 * Checks if all data for a date is empty
 * @param {Array} dateData - Array of day data for a specific date
 * @returns {boolean} - True if the date has no content
 */
function isEmptyDate(dateData) {
    if (!dateData || !Array.isArray(dateData)) return true;
    
    for (const dayData of dateData) {
        if (dayData && Array.isArray(dayData) && dayData.some(cell => cell && cell.trim() !== '')) {
            return false;
        }
    }
...
// Function to silently save the current state before switching weeks
async function saveTimeTableSilently() {
    if (!currentTimetableName || !timetables[currentTimetableName]) {
        return;
    }
    ...
    rows.forEach((row, dayIndex) => {
        ...
        // Save all cell contents (not just edited ones)
        cells.forEach((cell, hourIndex) => {
            timetables[currentTimetableName].data[dateString][dayIndex][hourIndex] = cell.textContent.trim();
    ...
    // No UI updates or messages - just save in the background
    try {
        await saveTimetable(currentTimetableName, timetables[currentTimetableName]);
    } catch (error) {
...
function setupCellEditing() {
    ...
    cells.forEach(cell => {
        ...
        // Add new event listener with content length limit
        newCell.addEventListener('input', function() {
            ...
            if (this.textContent.length > 30) {
                const truncatedContent = this.textContent.substring(0, 30);
                this.textContent = truncatedContent;
                this.title = truncatedContent; // Add tooltip with full content
            }
        });
        ...
        newCell.addEventListener('keydown', function(e) {
            if (e.key === 'Enter') {
                e.preventDefault();
                // Instead of new line, add a space if at the end
                if (window.getSelection) {
                    const sel = window.getSelection();
                    if (sel.focusNode === this.firstChild && 
                        sel.focusOffset === this.textContent.length) {
...
function performLogout() {
    ...
    // Show calendar placeholder message
    const calendarPlaceholder = document.querySelector('.calendar-placeholder');
    if (calendarPlaceholder) {
    ...
    // Add not-logged-in class to body to hide edit button and calendar
    document.body.classList.add('not-logged-in');
}
...
function togglePermanentHourMode() {
    ...
    // Update UI to show which cells are permanent
    const cells = document.querySelectorAll('.week-table tbody td:not(:first-child)');
    cells.forEach(cell => {
        if (cell.classList.contains('permanent-hour')) {
            // Enhance permanent hour styling
            cell.style.backgroundColor = permanentHourModeEnabled ? '#fff3e0' : ''; // Light orange when mode is on
            cell.style.border = permanentHourModeEnabled ? '2px solid #ff9800' : ''; // Orange border when mode is on
        }
    });
    
    // Show feedback to user with updated information about propagation
    showCustomAlert('Admin Mode', 
        permanentHourModeEnabled ? 
        'Permanent hours mode is now ON. Edited cells will become permanent and will be copied to future weeks (up to 2 months ahead). Ctrl+click on cells to toggle permanent status.' : 
