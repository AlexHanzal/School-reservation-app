...
function toggleDatePicker() {
    console.log('Toggle date picker called');
    ...
    if (!datePickerOverlay || !datePickerWindow) {
        console.error('Date picker elements not found');
        return;
    }

    const isVisible = datePickerOverlay.style.display === 'block';
    ...
    if (isVisible) {
        ...
        // Update the title
        const pickerTitle = datePickerWindow.querySelector('h2');
        if (pickerTitle) {
            pickerTitle.textContent = `Set Current Date - ${translations[currentLanguage].months[currentMonth]} ${currentYear}`;
        }
    }
}
