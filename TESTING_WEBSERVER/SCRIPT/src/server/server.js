...
// Create router instead of app for integration
const router = express.Router();

// Configuration - use relative paths within SCRIPT directory
const SCRIPT_BASE = path.resolve(__dirname, '../..');
const DATA_DIR = path.resolve(SCRIPT_BASE, 'data/timetables');
const USERS_DIR = path.resolve(SCRIPT_BASE, 'data/Users');
...
// Serve static files from the src directory for this project
router.use('/static', express.static(path.join(SCRIPT_BASE, 'src')));

// Root route for the school reservation app
router.get('/', (req, res) => {
    res.sendFile(path.join(SCRIPT_BASE, 'src/outside/index.html'));
});
...
router.put('/api/timetables/:name', async (req, res) => {
    try {
